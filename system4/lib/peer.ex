...
defmodule Peer do
  ...
  def start() do
    ...
    pl = Node.spawn(:'node1@container1.localdomain', LPL, :start, [])
    beb = Node.spawn(:'node1@container1.localdomain', BEB, :start, [])
    app = Node.spawn(:'node1@container1.localdomain', App, :start, [])
    next(pl, beb, app, nil)
  end

  defp next(pl, beb, app, system4) do
    receive do
      {:start, system4} ->
        send(system4, {:bind, self(), app, pl})
        next(pl, beb, app, system4)
      {:switch, app_pl} -> 
        send(app, {:bind, Map.keys(app_pl), system4, beb})
